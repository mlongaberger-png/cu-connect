...
import { Trophy, Users, Calendar, UserCircle, TrendingUp, ClipboardList, DollarSign } from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: submissions = [] } = useQuery({ queryKey: ["reg-submissions-all"], queryFn: () => base44.entities.RegistrationSubmission.filter({ status: "pending" }) });
  const { data: payments = [] } = useQuery({ queryKey: ["payments-dashboard"], queryFn: () => base44.entities.Payment.list() });
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      {/* Animated Performance Hero — headline of the dashboard */}
      <PerformanceHero
        events={events}
        teams={teams}
        sports={sports}
