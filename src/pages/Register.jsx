...
export default function Register() {
  ...
  // Base44's platform email/password signup screen (the path used by anyone
  // who doesn't sign up with Google/Microsoft/Apple) has no name field of its
  // own -- it defaults user.full_name to the local part of the email address
  // (e.g. "jane.smith" from jane.smith@gmail.com). Blindly trusting that here
  // meant email/password parents got a garbage "Full Name" silently pre-filled
  // and, if they didn't notice and retype it, submitted with their application
  // (verified live: a test signup saved parent_name "mtlongaberger+cu_signup1"
  // straight from the email). Only pre-fill when full_name looks like an
  // actual name -- i.e. it doesn't just match the email's local part -- so an
  // auto-generated value falls back to blank and the parent has to type their
  // real name instead.
  useEffect(() => {
    if (user) {
      const emailLocalPart = (user.email || "").split("@")[0].toLowerCase();
      const looksAutoGenerated =
        user.full_name && user.full_name.toLowerCase() === emailLocalPart;
      setParent(p => ({
        parent_name: p.parent_name || (looksAutoGenerated ? "" : user.full_name) || "",
