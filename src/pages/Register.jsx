...
// Base44's platform email/password signup screen has no name field of its
// own, so it defaults user.full_name to the local part of the email address
// -- and Apple sign-in can also come back with no name if the parent hides
// it. Either way, when full_name doesn't look like a real, provider-verified
// name we know less about this parent than usual, so we ask for a phone
// number too (Google sign-in always gives us a real name and we leave phone
// optional, same as elsewhere in the app).
function hasVerifiedName(user) {
  if (!user || !user.full_name) return false;
  const emailLocalPart = (user.email || "").split("@")[0].toLowerCase();
  return user.full_name.toLowerCase() !== emailLocalPart;
}

function needsPhoneNumber(user) {
  return !!user && !hasVerifiedName(user);
}

function deriveParentDefaults(user) {
  if (!user) return { parent_name: "", parent_email: "", parent_phone: "" };
  return {
    parent_name: hasVerifiedName(user) ? (user.full_name || "") : "",
...
export default function Register() {
  ...
  // Pre-fill parent fields from auth (see hasVerifiedName/deriveParentDefaults
  // above for why full_name isn't trusted blindly, and why email/password
  // signups get asked for a phone number).
  useEffect(() => {
    if (user) {
      const defaults = deriveParentDefaults(user);
      setParent(p => ({
        parent_name: p.parent_name || defaults.parent_name,
        parent_email: p.parent_email || defaults.parent_email,
        parent_phone: p.parent_phone || defaults.parent_phone,
      }));
    }
  }, [user]);
  ...
  const isValid = () =>
    ...
    (!needsPhoneNumber(user) || parent.parent_phone.trim()) &&
    athletes.length > 0 &&
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await base44.functions.invoke("submitRegistrationApplication", {
        ...
        parent_phone: parent.parent_phone?.trim() || undefined,
  ...
  const resetForm = () => {
    ...
    setReferralSource("");
    setReferralNote("");
    setParent(deriveParentDefaults(user));
  };
      ...
      <div className="max-w-xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
            ...
            <div>
              <Label>
                Phone Number {needsPhoneNumber(user) ? "*" : <span className="text-muted-foreground font-normal">(optional)</span>}
              </Label>
              <Input type="tel" value={parent.parent_phone} onChange={e => setParent(p => ({ ...p, parent_phone: e.target.value }))} className="bg-surface border-border" required={needsPhoneNumber(user)} />
              {needsPhoneNumber(user) && (
                <p className="text-xs text-muted-foreground mt-1">Since you signed up with email/password, a phone number gives coaches and admins another way to reach you.</p>
              )}
            </div>
          </div>
