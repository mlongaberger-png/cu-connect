...
const FINDINGS = [
  ...
  },
  {
    ...
    fix: "Introduced UserSession entity with SHA-256 token hashing. Added validateSession gate to all sensitive write functions.",
  ...
  },
  {
    ...
    vuln: "content_text, sender_name, and channel_id were interpolated raw into HTML email bodies. A crafted message containing <script> tags would execute in recipient email clients.",
  ...
  },
  {
    ...
    fix: "trackSession now forces creation of a new session record on every login. Revoked records are never reactivated.",
  ...
  },
  {
    ...
    fix: "Added Payment entity RLS: parents read only records where parent_email matches their own email. Staff roles retain full read access.",
  ...
  },
  {
    ...
    fix: "icsCalendarFeed enforces: token presence check → 401 if absent, SHA-256 hash lookup → 401 if not found or revoked, 90-day expiry check with auto-revocation.",
  ...
  },
  {
    ...
    fix: "generateCalendarToken revokes all existing active CalendarToken records for the user before creating a new one.",
  ...
  },
  {
    ...
    fix: "Created requireAdminAuth() helper that calls base44.entities.User.get() to verify role from the database. All attempts logged to AdminAuditLog.",
  ...
  },
  {
    ...
    fix: "All write functions now derive team scope exclusively from the authenticated session. Client-supplied team_id in POST bodies is ignored. Zod .strict() schemas prevent unexpected field injection.",
...
function buildPDF() {
  ...
  divider([200, 168, 75]);
  para("This report documents the identification, remediation, and verification of all ten (10) security findings discovered during the Phase 1 security assessment of the CU Connect platform.");
  y += 4;
  ...
  label("Out of Scope — Phase 2", [160, 80, 20]);
  ...
  doc.text("All P3 and P4 findings are deferred to a separate Phase 2 engagement.", margin + 12, y + 32);
  doc.text("This report provides clearance for beta launch based on Phase 1 scope only.", margin + 12, y + 44);
  y += 64;

  divider();
  para("This report was generated from the CU Connect platform security audit record. All findings, evidence, and remediation steps reflect actual code changes. This document is classified CONFIDENTIAL and intended for internal use only.", { color: [130, 130, 130], size: 8.5 });
...
export default function SecurityReport() {
  ...
  const handleGenerate = () => {
    ...
    try {
      const doc = buildPDF();
      doc.save("CU_Connect_Final_Security_Remediation_Report_v1.0.pdf");
    } finally {
      setLoading(false);
    }
  };
