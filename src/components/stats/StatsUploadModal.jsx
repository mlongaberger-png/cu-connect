...
export default function StatsUploadModal({ open, onOpenChange, player, teamId, teamName }) {
        ...
        {status === "idle" && (
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              Upload a GameChanger screenshot, PDF, or photo of a stat sheet for{" "}
              <span className="text-foreground font-medium">{player?.first_name} {player?.last_name}</span>.
            </p>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleProcess} disabled={!file} className="gap-1.5">
                <Sparkles className="w-4 h-4" /> Extract Stats
              </Button>
            </div>
          </div>
        )}
        ...
        {(status === "uploading" || status === "extracting") && (
          <div className="py-10 flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-muted-foreground text-center">
              {status === "uploading" ? "Uploading file…" : "AI is reading the stats sheet…"}
            </p>
          </div>
        )}
        ...
        {status === "done" && result && (
          <div className="py-4 space-y-4">
            <div className="flex items-center gap-2 text-green-400">
              ...
              <span className="font-medium">Stats extracted successfully!</span>
            </div>
            <div className="text-sm text-muted-foreground space-y-1">
              <p>Categories found: <span className="text-foreground font-medium capitalize">{result.stat_types?.join(", ") || "—"}</span></p>
              <p>Records saved: <span className="text-foreground font-medium">{result.records_created}</span></p>
            </div>
