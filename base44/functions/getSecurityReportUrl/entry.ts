...
Deno.serve(async (req) => {
  try {
    ...
    const divider = () => { if (y > 740) { doc.addPage(); y = 50; } doc.setDrawColor(180, 180, 180); doc.line(50, y, pageW - 50, y); y += 16; };
    ...
    bullet('Checks: revoked_at (null), last_used_at (<24h), created_date (<7d), rotated_at (<4h).');
    ...
    bullet('Returns HTTP 200 with reason: "session_not_found" for missing records.');
    ...
    bullet('All gated functions individually tested to confirm gate behavior.');
    y += 6;
    ...
    const pdfFile = new File([pdfBytes], 'CU_Connect_Security_Phase1_2026-06-17.pdf', { type: 'application/pdf' });
    ...
    if (!fileUri) {
      return Response.json({ error: 'Upload failed, no file_uri returned', details: uploadRes }, { status: 500 });
    }
